...
```{r, echo=FALSE}
ggplot(combined_avg_price, aes(x = change_zillow, y = change_airbnb)) +
  geom_point() +
  # geom_text(aes(label = shortNeighborhood), vjust = -0.5, hjust = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Percent Change in Zillow vs Airbnb Prices by Neighborhood",
       x = "Percent Change in Zillow Prices",
       y = "Percent Change in Airbnb Prices") +
  theme_minimal()
```
...
```{r, echo=FALSE}
# Fit a linear model
fit <- lm(change_airbnb ~ change_zillow, data = combined_avg_price)
